...
public class TblCont extends Table implements iGenID
{
   ...
   */
  public TblCont(ConnectionPool conn, MsSqlPool msp, Map map)
  {
    super(conn, msp, map);
  }
   ...
   * @param map Lista parametri da caricare
   */
  public TblCont(ConnectionPool conn, Map map)
  {
    super(conn, map);
  }
  ...
  /**
   * Questo metodo viene utilizzato per calcolare la chiave univoca per la chiave 
   * primaria
   * @param code codice che indica la tabella per la quale generare il codice
   * @param length indica la lunghezza della chiave
